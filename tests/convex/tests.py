...
class TestConvexVirtualBalanceRewardPoolContract:
    ...
    _address = '0x81fCe3E10D12Da6c7266a1A169c4C96813435263'
    _provider = BridgeConfigurator(abstractFabric=providerAbstractFabric, fabricKey='http', productKey=Chains.ETH).produceProduct()

    _instance = ConvexVirtualBalanceRewardPoolContract()\
        .setAddress(address=_address)\
        .setProvider(provider=_provider)\
        .create()

    def testInstance(self):
        assert isinstance(self._instance, ConvexVirtualBalanceRewardPoolContract)
    ...
    def test_deposits(self):
        assert isinstance(self._instance.deposits(), str)
